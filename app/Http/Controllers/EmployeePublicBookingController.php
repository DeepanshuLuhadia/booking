...
class EmployeePublicBookingController extends Controller
{
    ...
    public function show($identifier, Request $request, SlotGenerationService $slotService)
    {
        ...
        // The distance-warning popup is a client-side precheck: the frontend
        // hits this same route with an AJAX request before navigating, and
        // only shows the "vendor is far away" confirmation if this responds
        // with a warning. A normal (non-AJAX) page load must never receive
        // raw JSON here, or the browser renders it as text on a blank page —
        // so it always falls through to the real page below, in range or not.
