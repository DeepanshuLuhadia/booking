<x-app-layout :vendor-theme="$theme" :page-title="$vendor->business_name">
    <div x-data="bookingSystem()" class="relative min-h-screen text-white vendor-theme--{{ strtolower(str_replace(' ', '-', $theme['label'] ?? 'default')) }}">
        ...
        <section class="relative z-10 pt-32 pb-16 px-6">
            <div class="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-12">
                ...
                <div class="relative group shrink-0">
                    <div class="w-64 h-64 md:w-80 md:h-80 rounded-[3rem] overflow-hidden theme-glow-border transition-transform duration-1000 group-hover:scale-105 mx-auto">
                ...
                <div class="flex-grow text-center md:text-left animate-text-reveal">
                    <div class="inline-flex items-center gap-2 px-4 py-1.5 bg-white/10 border border-white/20 backdrop-blur-xl rounded-full text-white/50 text-[9px] font-black uppercase tracking-widest mb-6">
                        <span class="w-2 h-2 rounded-full theme-gradient-bg animate-pulse"></span> Verified Appointment Registry
                    </div>
                    ...
                    <div class="flex flex-wrap items-center justify-center md:justify-start gap-8 mb-10">
                        <a href="https://www.google.com/maps/search/?api=1&query={{ urlencode($vendor->address ?? 'Professional District') }}" 
                           ...
                           class="flex items-center gap-3 group/address transition-all hover:scale-[1.02]">
                            <div class="w-10 h-10 rounded-xl bg-white/10 backdrop-blur-xl flex items-center justify-center theme-gradient-text border border-white/10 group-hover/address:border-white/30 group-hover/address:bg-white/20 transition-all">
                                <svg class="w-5 h-5 text-current" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/></svg>
                            </div>
                            <span class="text-base font-bold text-white/60 italic group-hover/address:text-white transition-colors underline underline-offset-4">{{ $vendor->address ?? 'Professional District' }}</span>
                        </a>
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-xl bg-white/10 backdrop-blur-xl flex items-center justify-center theme-gradient-text border border-white/10">
                    ...
                    <div class="flex flex-wrap gap-2 justify-center md:justify-start">
                        <span class="theme-pill theme-gradient-bg border-transparent text-white font-black uppercase tracking-widest">{{ strtoupper($theme['label']) }} EXPERT</span>
                        @foreach($theme['services'] as $service)
                            <span class="theme-pill text-[9px] font-black uppercase tracking-widest text-white/60">{{ $service }}</span>
                        @endforeach
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-12 xl:col-span-7">
                <div class="mb-12">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <span class="theme-gradient-text font-black text-[10px] uppercase tracking-widest italic">Booking Step 01</span>
                    </div>
                ...
                <div class="space-y-4">
                    @forelse($vendor->employees as $employee)
                        <button @click="fetchSlots({{ $employee->id }}, {{ $employee->service_fee_override ?? $vendor->service_fee }})"
                                ...
                                :class="selectedEmployee === {{ $employee->id }} ? 'bg-white/5 theme-glow-border scale-[1.02] opacity-100 z-10' : 'bg-white/5 border-transparent opacity-50 hover:opacity-80 hover:border-white/20'">
                            ...
                            <div class="flex-grow">
                                <h4 class="text-2xl font-black italic transition-colors"
                                    :class="selectedEmployee === {{ $employee->id }} ? 'theme-gradient-text' : 'text-white'">{{ $employee->name }}</h4>
                                <div class="flex items-center gap-4 mt-2">
                                    ...
                                    @if($employee->service_fee_override)
                                        <span class="px-3 py-1 theme-gradient-bg text-white rounded-lg text-[8px] font-black uppercase tracking-widest shadow-sm">Premium Talent</span>
                                    @endif
                                </div>
                            </div>
                ...
                <div class="mt-24 pt-24 border-t border-white/5">
                    ...
                    <div class="glass-card shadow-xl shadow-black/20 bg-white/5 backdrop-blur-3xl border border-white/10 p-10 text-lg font-medium text-white/60 leading-relaxed italic" style="padding: 24px; border-radius: 16px;">
                ...
                <div class="sticky top-32">
                    <div class="glass-card shadow-2xl shadow-black/20 bg-white/10 backdrop-blur-3xl border-white/10 p-2 overflow-hidden rounded-[3rem]">
                        <div class="p-8 pb-4 flex items-center justify-between">
                            <div>
                                <span class="theme-gradient-text font-black text-[10px] uppercase tracking-widest italic block mb-2">Step 02</span>
                                <h3 class="text-3xl font-black text-white tracking-tighter italic" x-text="isTokenEnabled ? 'Choose Token & Wait Time' : 'Choose Time'"></h3>
                            </div>
                            ...
                            <div x-show="!loading && selectedEmployee" class="animate-reveal">
                                        ...
                                        <div class="grid grid-cols-2 gap-3">
                                            <template x-for="slot in slots" :key="slot.start">
                                                <button @click="initiateBooking(slot)"
                                                    ...
                                                    <span class="text-xl md:text-2xl font-black italic tracking-tighter text-white block transition-colors" x-text="slot.start"></span>
                                ...
                                <template x-if="isOffline">
                                    <div class="py-20 px-8 text-center bg-white/5 rounded-[2.5rem] border border-white/10 animate-reveal">
                                        <div class="w-20 h-20 theme-gradient-bg rounded-3xl flex items-center justify-center mx-auto mb-6 border theme-border opacity-50">
                                        ...
                                        <div class="inline-block px-6 py-2 bg-white/10 rounded-xl border border-white/10">
                                            <span class="theme-gradient-text font-black text-xs uppercase italic tracking-widest">Opens At: <span x-text="openingTime"></span></span>
                                        </div>
                                    </div>
                                </template>
                            </div>
        ...
        <div x-show="bookingModal" class="fixed inset-0 z-[200] flex items-center justify-center p-6" x-cloak x-transition>
            ...
            <div class="relative bg-slate-900/90 text-white rounded-[2rem] sm:rounded-[4rem] p-6 sm:p-12 text-center w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-[0_100px_200px_-50px_rgba(0,0,0,0.8)] border border-white/10">
                <div class="mb-10">
                    <span class="inline-block px-4 py-1 theme-gradient-bg text-white border theme-border rounded-full text-[9px] font-black uppercase tracking-widest italic mb-6">Security Clearance</span>
                ...
                <div class="space-y-5 text-left mb-10">
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            ...
                            <input type="text" x-model="guestName" class="premium-input w-full h-14 pl-12 text-base bg-white/5 border-white/10 text-white placeholder-white/20 theme-focus-border" placeholder="Full {{ $theme['customer_label'] }} Name">
                        </div>
                    </div>
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            ...
                            <input type="tel" x-model="guestPhone" maxlength="10" class="premium-input w-full h-14 pl-12 text-base bg-white/5 border-white/10 text-white placeholder-white/20 theme-focus-border" placeholder="10 Digit Primary Number">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-black/40 rounded-[2.5rem] p-10 text-white mb-10 shadow-2xl relative overflow-hidden border border-white/5">
                    ...
                    <div class="relative z-10 space-y-4">
                        ...
                        <div x-show="selectedSlot?.requires_emergency" class="flex justify-between items-center theme-gradient-text">
                        ...
                        <div class="flex justify-between items-center pt-6 border-t border-white/10">
                            ...
                            <span class="text-4xl font-black theme-gradient-text drop-shadow-[0_0_10px_rgba(255,255,255,0.2)]" x-text="'₹' + totalAmount"></span>
                        </div>
                    </div>
                </div>
