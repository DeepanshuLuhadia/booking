<x-vendor-layout>
    <div class="flex flex-col md:flex-row items-center justify-between gap-8 mb-12">
        <div>
            <h1 class="text-4xl font-black italic tracking-tight uppercase text-slate-900">Appointment <span class="text-blue-600">Registry.</span></h1>
            <p class="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mt-2 italic">HISTORICAL TRANSACTION ARCHIVE</p>
        </div>
        <div class="w-full md:w-auto">
            <form action="{{ route('vendor.bookings.index') }}" method="GET">
                <div class="relative group">
                    <select name="status" onchange="this.form.submit()" class="w-full md:w-64 h-14 bg-white border-2 border-slate-100 rounded-xl px-6 font-black italic text-slate-900 focus:ring-4 focus:ring-blue-50 focus:border-blue-600 transition-all appearance-none cursor-pointer text-[10px] uppercase tracking-widest">
                        <option value="">Matrix: All States</option>
                        <option value="confirmed" {{ request('status') == 'confirmed' ? 'selected' : '' }}>State: Confirmed</option>
                        <option value="completed" {{ request('status') == 'completed' ? 'selected' : '' }}>State: Completed</option>
                        <option value="cancelled" {{ request('status') == 'cancelled' ? 'selected' : '' }}>State: Cancelled</option>
                    </select>
                    <div class="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-slate-300 group-hover:text-blue-600 transition-colors">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="4" d="M19 9l-7 7-7-7"/></svg>
                    </div>
                </div>
            </form>
        </div>
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($bookings as $booking)
                        <tr class="hover:bg-slate-50/50 transition-all">
                            ...
                            <td class="px-10 py-8">
                                <div class="text-sm font-black text-slate-900 italic tracking-tight">{{ \Carbon\Carbon::parse($booking->booking_date)->format('l, M d, Y') }}</div>
                            ...
                            <td class="px-10 py-8 text-right">
                                <button class="p-3 bg-slate-50 text-slate-300 rounded-xl hover:text-blue-600 transition-all">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"/></svg>
                                </button>
                            </td>
                        </tr>
                    @empty
